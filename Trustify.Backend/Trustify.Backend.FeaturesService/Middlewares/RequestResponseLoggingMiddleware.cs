...
using Trustify.Backend.FeaturesService.Logger;

namespace Trustify.Backend.FeaturesService.Middlewares
{
    public class RequestResponseLoggingMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, ILogger<RequestResponseLoggingMiddleware> logger)
        {
            ...
            try
            {
                ...
                using (LogContext.PushProperty(LogContextConstants.Username, "nousername"))
                using (LogContext.PushProperty(LogContextConstants.Method, httpContext.Request.Method))
                using (LogContext.PushProperty(LogContextConstants.RequestPath, httpContext.Request.Path))
                using (LogContext.PushProperty(LogContextConstants.Protocol, httpContext.Request.Protocol))
                using (LogContext.PushProperty(LogContextConstants.QueryString, httpContext.Request.QueryString))
                using (LogContext.PushProperty(LogContextConstants.ContentType, httpContext.Request.ContentType))
                {
